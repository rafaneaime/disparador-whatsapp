    ...
    <Cartao destaque className="p-5">
      ...
      <form onSubmit={cadastrar} className="mt-5 flex flex-col gap-3">
        <label className="text-sm font-medium">
          Um por linha: telefone, ou nome e telefone
          <textarea value={texto} onChange={(evento) => { setTexto(evento.target.value); setResultado(null); }}
            rows={7} placeholder={'Maria Silva, (11) 98765-4321\n11 91234-5678, João Souza\n+1 202 555 0123'}
            disabled={carregando} className={`mt-1 w-full ${ESTILO_CAMPO}`} aria-describedby="limite-colagem" />
        </label>
        <p id="limite-colagem" className="text-xs text-tinta-fraca">
          O nome é opcional, e vem antes ou depois do telefone, separado por vírgula, ponto e vírgula ou tabulação —
          dá para colar direto da planilha. Até 1.000 linhas por vez. Para números de fora do Brasil, inclua + e o DDI.
        </p>
      ...
      {resultado && (
        <div className="mt-5 rounded-xl border border-linha-forte p-4">
          ...
          <p className="mt-2 text-xs text-tinta-fraca">
            Contatos existentes mantêm o consentimento anterior, e quem já tem nome mantém o nome. Colar novamente
            não reativa descadastrados.
          </p>
              ...
              <li key={indice} className="break-words rounded-lg bg-papel p-2">
                ...
                {linha.situacao === 'invalido' ? `Inválido: ${linha.motivo}`
                  : linha.situacao === 'repetido' ? `Repetido nesta colagem: ${linha.telefone}`
                    : `${linha.resultado === 'cadastrado' ? 'Cadastrado' : 'Já existente, preservado'}: ${linha.telefone}${linha.nome ? ` · ${linha.nome}` : ''}`}
              </li>
            ))}
          </ol>
        </div>
